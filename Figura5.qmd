...
glist <- read_tsv("data/raw_data/genome_status_supplementary_tableS2.txt.gz", col_names = T) #84,022  7
arg_mge <- read_tsv("data/raw_data/mge_arg_final.txt.gz", col_names = T) #1,499,222  4
arg_genome <- read_tsv("data/raw_data/genome_arg.txt.gz", col_names = F) # 72389823  4
```
...
dplyr::rename(abr_count = ...3, total = ...4) %>% 
...
arg_genome_filtered <- arg_genome %>% 
...
arg_combined <- rbind (arg_mge_filtered, arg_genome_filtered)
...
1. Renombra columnas: abr_total a abr, all a non_abr.

2. Convierte a formato largo con melt() (tendrás dos filas por MGE: una para abr, otra para non_abr).

3. Usa spread() para convertirlo a formato ancho:

  - Columnas = tipos de MGE.

  - Filas = abr y non_abr.

  - Valores = conteos absolutos.
  
Guadar datos

```{r}
pre_arg_combined_filtered_m
```
...
`m_arg_rm_final` es una tabla limpia y lista para analizar asociaciones entre mecanismos de resistencia y MGEs. Contiene columnas como:

  - prot = proteína ARG.

  - mech o mecanismo de resistencia (según tengas definido).

  - mge = tipo de MGE (por ejemplo, IS_Tn, Phage, Integron, etc.).
...
together_rm <- rbind(m_arg_rm_final,g_arg_rm_final) %>% 
  ...
  # Seleccionar la ontología de mecanismo relevante entre participates_in e is_a
...
for (i in allClasses) {
  for (j in allMges) {
    print(paste0(i, "_", j))
    inarg <- arg_rm[i, j]                                   # ARG en MGE específico
    outarg <- sum(arg_rm[, j]) - inarg                      # Otros ARG en ese MGE
    notmge <- sum(arg_rm[i, ]) - inarg                      # Ese ARG en otros MGEs
    outargoutmge <- sum(arg_rm) - inarg - outarg - notmge   # Todo lo demás
...
FE_arg_adj <- matrix(
  p.adjust(p = FE_arg, method = "bonferroni"), 
  ncol = ncol(FE_arg), nrow = nrow(FE_arg), 
  dimnames = list(rownames(FE_arg), colnames(FE_arg))
)

# Paso 4: Preparar tabla larga con significancia y anotaciones
FE_arg_adj_t <- as_tibble(FE_arg_adj) %>% 
...
arg_hmap <- ggplot(m_FE_arg_adj_t_rs, aes(x = reorder(mge, value, sum), y = reorder(abr, value, sum), fill = log10(value))) + 
  geom_tile(colour = "black") + 
  geom_text(aes(label = signP), colour = "white", size = 8) + 
  scale_fill_gradientn(colours = viridis(10), values = rescale(c(0, 3, 4, 6)), na.value = "grey80")

# Paso 7: Personalizar tema de la figura
arg_hmap + 
  theme_cowplot(font_size = 20) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  labs(x = "", y = "", fill = "#ABR genes")
```
